...
@ray.remote
def get_res_damping_noise_core(param, problem,problem_scale):
    opt = CobylaOptimizer(max_iter=200)
    aer = PhaseAmplitudeDampingNoiseAerProvider(param)
    solver = ChocoSolver(
    ...
    with open('damping_noise_res.csv', 'a') as f:
        f.write(f'{problem_scale},{param},{scc},{incons},{ARG},{num_bases}\n')

def get_res_damping_noise_ray(num_case=10, problem_scale_list=[(2, 2)]):
    problems, _ = generate_flp(num_case,problem_scale_list, 1, 100)
    processes = []
        
    with open('damping_noise_res.csv', 'w') as f:
        f.write('problem_scale,damping_param,success_rate,inconsistency_rate,ARG,num_bases\n')
    # time unit : ns
    ...
    for k in range(len(problem_scale_list)):
        for param in param_lst:
            for i in range(num_case):
                ...
                processes.append(get_res_damping_noise_core.remote(param ,problem,problem_scale_list[k]))
    ray.get(processes)

if __name__ == '__main__':
    get_res_damping_noise_ray(num_case=100, problem_scale_list=[(2,1),(2, 2),(2,3),(3,1),(3, 2)])


# screen -L -S damping /home/xdb/miniconda3/envs/qiskit1env310/bin/python /home/xdb/quproj/Choco/1_noise_damping.py
